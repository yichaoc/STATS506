...
Our group mainly focuses on the relatinship between diet habit (specifically,
...
We also use demographic variables (gender, family povery income ratio, and age)
as additional covariates.

Based on our data, we chose to use zero-inflated negative binomial regression
to fit the model. The choice of this regression is further illustrated
our method section. Our group members used Stata,R and SAS.  
...
#### Basic analysis
The following processes is done using SAS.
...
```{r zinbreg, echo=FALSE, fig.align='center',out.width = 609, out.height = 760,echo=FALSE}
...
healthfulness of a person's diet and their alcoholic consumption, indicating that people with less healthy
diets tend to drink more alcohol on average. Specifically, the expeceted change in log(alq_drink) for one-unit increase in diet is 0.08 holding other variables constant. From the codebook, the larger diet factor indicate poorer diet behavior. That is to say, we have evidence to show that people who drink and have healthier diet tend to drink more.

Hold other variables the same, log(alq_drink) is 0.5 larger when gender = 1(male) than gender = 0(female), indicating that among drinking people, men tend to drink more than women do.

The expected change in log(alq_drink) for one-unit increase in age is -0.02 holding other variables constant. This means among drinking people, older people tend to drink less alcohol.

The expected change in log(alq_drink) for one-unit increase in family poverty income ratio is -0.10 holding other variables constant, which means that means among drinking families, wealtheir families consume less alcohol on average.

The log odds of being an excessive zero will decrease by 0.06 for every one more meal eating outside. This means when the frequency of eating out of home is larger, the zero are less likely comes from the part of people who never use alcohol. In other words, more meals eating out of home implies more alcohol use.
